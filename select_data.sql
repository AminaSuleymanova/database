...
a.building, a.flat from usr u inner join address a on u.id=a.user_id 
select u.first_name, u.middle_name, u.last_name, a.zip_code, a.city, a.street, 
...
from university_member um join usr m on um.user_id=m.id join position p on um.position_id=p.id
...
select m.first_name, m.middle_name, m.last_name, m.e_mail, d.name from university_member um 
join usr m on um.user_id=m.id join department d on um.department_id=d.id;
